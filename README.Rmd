...
In accordance to the FAIR (Findable, Accessible, Interoperable, Reusable) principles of open-science, you will find in the `jk.fanf.tarping` repository all the **data and codes** we used to produce the results presented in the manuscript we submitted to *Biological Invasions* and that is currently called "What drives the success of knotweed control by tarping?". Upon acceptance, we will update this file to link it to the DOI of the article (if you intend to reuse this dataset, please cite the associated article in your own communications).
